...
def _install_mcp() -> bool:
    """Register the MCP server with Claude Code. Returns True if action taken."""
...
def _install_skill() -> bool:
    """Create skill symlink. Returns True if action taken."""
    if not SKILL_DIR.is_dir():
        print(f"  Warning: skill directory not found at {SKILL_DIR}")
        return False
...
def _uninstall_mcp() -> bool:
    """Remove MCP server registration. Returns True if action taken."""
    result = _run_claude(
